...
if __name__ == "__main__":
    ...
    print(embed.shape)
    print("Done")
    sentence_model = SentenceTransformer("paraphrase-albert-small-v2").to("cuda:1")
    embed2 = sentence_model.encode(test_sentence,convert_to_numpy=True) 
    print(type(embed2))
    print(embed2.shape)
    print("Done2!")
    cos = nn.CosineSimilarity(dim=0)
    print(cos(torch.tensor(embed),torch.tensor(embed2)))