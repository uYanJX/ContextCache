...
class DynamoStorage(CacheStorage):
            ...
            for item in all_data:
                ...
                batch.put_item(
                    Item = self._strip_all_props_with_none_values({
                        ...
                        "context_data": DynamoBinary(item.context_data.tobytes()) if item.context_data is not None else None,
    ...
    def _response_item_to_cache_data(self, question_resp: Dict) -> Optional[CacheData]:
        ...
        return CacheData(
            ...
            context_data = np.frombuffer(question_resp["context_data"].value, dtype=np.float32),
