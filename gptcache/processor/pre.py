...
def last_content(data: Dict[str, Any], **_: Dict[str, Any]) -> Any:
    ...
    if isinstance(data.get("messages")[-1]["content"],str):
        return data.get("messages")[-1]["content"], None
    else:
        #  return " ".join(data.get("messages")[-1]["content"]), data.get("messages")[-1]["content"][:-1] # data.get("messages")[-1]["content"][-1],
        return data.get("messages")[-1]["content"][-1], data.get("messages")[-1]["content"][:-1] # " ".join(data.get("messages")[-1]["content"])

def all_content_list(data: Dict[str, Any], **_: Dict[str, Any]) -> Any:
    ...
    s = [message["content"] for message in messages if message["role"] == "user"]
    return s
